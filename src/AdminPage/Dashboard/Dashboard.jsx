...
import { faCar,faArrowUp, faArrowDown, faCircle } from '@fortawesome/free-solid-svg-icons';
...
function Dashboard() {
  ...
  const fetchListCar = async () => {
    try {
      const res = await fetch("https://cnpm-ncserver.vercel.app/api/getVehicleByAdmin");
      if (!res.ok) {
...
const handleFilter = () => {
  ...
  const seatTypes =[... new Set(car.map((item) => item.Number_Seats))].map(seat => `${seat} ghế`);
  const seatType =[... new Set(car.map((item) => item.Number_Seats))] // Lấy ra mảng chứa các loại ghế không trùng lặp.
  const carCountPerSeat = seatType.map(seat => car.filter(item => item.Number_Seats === seat).length); // Đếm số lượng xe cho từng loại ghế
